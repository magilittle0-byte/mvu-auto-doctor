...
export function buildActorShardMessages(candidate, {
    ...
    const system = [
        ...
        'persistentActorState.identity与hidden是人物DNA：行动应体现该角色自己的社交办法、决策办法、现实欲望、边界、习惯、盲点和压力应对，而不是仅由职业或本轮情绪驱动。强烈情绪不能抹掉其长期目标与日常行为；若档案字段仍为空，只按已有证据行动，不自行套入“冷酷、暴躁、绝望、怯懦、狂热”默认模板。',
