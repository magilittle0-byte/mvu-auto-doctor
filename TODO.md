...
> v1.8.0 已完成审计中适合当前架构的高、中优先级项目，并补做调用统计、提示词透明、
> 注入落地探针、脱敏诊断导出、通用正文包装识别和真实浏览器 CI。
> 下列条目保留 Fable 原始建议与完成状态，避免后续维护者重复实现或误解边界。

## 高优先级（性价比最高）

### 1. 环境自检卡（U10）— v1.8.0 已完成
...
### 8. index.js 模块化拆分（工程报告建议 1）— 延后到大版本
