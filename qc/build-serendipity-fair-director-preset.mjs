...
if (!sourcePath || !outputPath || !auditPath) {
    throw new Error(
        'usage: node qc/build-serendipity-fair-director-preset.mjs <source.json> <output.json> <audit.json>',
    );
}
...
const fullAudit = {
    ...audit,
    // The audit is a release artifact. Keep it reproducible and portable
    // without leaking a maintainer's local user or workspace path.
