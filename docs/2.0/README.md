...
状态：`2.0.0-rc.5-three-lane-pressure / real-qc-required`

适用范围：2.0 产品、数据协议、事务协议、真实故障回放与阶段交接

最后更新：2026-07-31

本目录是 2.0 实施的权威入口。阶段0冻结产品边界、协议、不变量、回放基线与交接规则；阶段1至6依次实现领域、事务、导演、领域事务、双入口和持久运行时。阶段7新增 `v2/release/` 的1.x升级/回滚演练、性能/容量/隐私、安全/恢复硬化门。阶段8—9提供 Actor Shard 领域核和宿主接线；rc.4 新增持久 Actor Ledger，rc.5 继续加入稳定身份揭示/异变谱系、生命周期、实际观察回写、人物/势力/环境三通道和共享压力/注入预算。历史阶段10阻断与修复记录仍保留作为事故证据，不代表当前提交已经通过新的真实门；rc.5 必须重新完成 [`../REAL_ENV_QC.md`](../REAL_ENV_QC.md) 后才可发布。
