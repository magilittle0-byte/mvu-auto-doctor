...
11. [`replay-fixture.schema.json`](replay-fixture.schema.json)：阶段0回放语料的机器可读 JSON Schema。
12. [`../../fixtures/2.0/replay-cases.json`](../../fixtures/2.0/replay-cases.json)：脱敏、最小化的真实故障回放基线。
...
第一条命令验证规格引用、schema、fixture结构、覆盖范围和隐私边界。第二条命令验证阶段1领域记录、1.x只读适配、未知字段保留，以及四个领域 `unit-active` 回放。第三条命令验证阶段2消息指纹、分支迁移、事务状态机、并发幂等、迟到结果和路径级保守回滚，以及两个事务 `unit-active` 回放。第四条命令验证阶段3 Turn Boundary、风险召回/语义裁决分离、H0–H3命令、Fact/Knowledge证据门、主模型上下文合同，以及五个导演层 `unit-active` 回放并保留错轮回复回归。第五条命令验证阶段4物品、装备、技能、社会、任务、H2复合事务和惰性迁移，并让五个领域事故 fixture 调用真实阶段4规划入口。宿主 UI、自然语言、数据库、下游屏障与真实环境行为仍按验收矩阵保持未激活，不会在默认 CI 中制造未来阶段的假失败。
