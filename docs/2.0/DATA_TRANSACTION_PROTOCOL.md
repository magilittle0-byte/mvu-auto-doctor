...
## 22. Actor Shard 提案与用户指令协议

Actor Shard 只能在当前完整目标已有持久 `settled` 屏障后启动。选择器依据连续性
线程阶段、紧迫度、关系、来源证据和角色是否在场确定性排序；默认关闭，启用时每回合
上限1—5、默认2。每个 worker 使用隔离上下文、可取消轻量调用和有限超时，不重试。

提案白名单固定为稳定角色ID/姓名、时间、地点、有限认知依据、当前目标、候选行动、
可能交互对象、来源线程、证据和因果链。额外字段、身份变化、越界来源、非完整JSON
或缺失硬字段一律拒绝。worker 不持有写适配器，不得修改 MVU、世界书、论坛、聊天、
数据库、事实或任务。

汇合器按稳定角色ID工作；只有时间、地点及共享来源/因果链全部兼容时才形成共同
候选，冲突保持独立并记录原因。结果只进入宏观连续性模型；原 continuity policy、
完整目标身份复核、元数据合并和 settled-only 协议仍是唯一写路径。

整个批次绑定 chat、message、swipe、generation、branch 与内容指纹，并以 TaskLease
提供进度、取消和超时。运行中 regenerate、swipe 或切聊天会让整批 stale 且输出为空；
单 worker 失败只增加诊断计数，不阻断旧宏观连续性路径。

两个自由提示词插槽最多各6000字符，只作为清楚标记的叙事/模拟指令注入对应模型。
