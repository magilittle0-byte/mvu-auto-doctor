...
- [x] `npm.cmd test`：171 pass、0 fail、0 todo、0 skip。
...
- [x] 远端候选分支存在；rc.3 完整源码和离线包已准备好。
- [ ] 默认分支 `main` 指向本次 rc.3 提交后，已有安装可直接点击自动医生这一行的
      “更新”，全新安装可将分支或标签留空。

## 真实环境门

- [x] SillyTavern 1.18.0/`8172dcd`，两处候选部署的62个运行时文件与候选源码一致。
...
- [x] 当前源码真实模型探针：`deepseek-v4-pro` 单次HTTP 200并正常完成；故意无效的
      社交审计结构被零写入拒绝，无 fallback、无关系删除，原始请求/响应未持久化。
