...
`automation.status=structural-only` 表示仍只有结构基线；`unit-active` 表示对应行为已经进入默认测试，但不单独冒充未执行的更高验收层。阶段1激活药剂、技能成本、装备槽位和强制/自愿关系分离；阶段2激活错轮回复与重Roll幂等；阶段3激活玩家负约束、三类口胡与随机暗号；阶段4激活普通善意；阶段5激活Android整帖展开；阶段6激活屏障、数据库与看门狗；阶段7激活真实QC优先发布门。17项fixture现在均有自动行为层，真实SillyTavern层仍必须由候选对应的QC收据证明。
