...
- 维护者于2026-07-27明确授权发布 `2.0.0-rc.1`；更新前 `main`
  `7d761ba6af1ceb10ab3ea5947c0bb25ee1b72566` 已固定到
  `codex/backup-main-pre-v2.0.0-rc.1-20260727`，随后 `main` 以
  `force:false` 非强制快进到候选远端提交。
...
维护者授权发布后，候选用户文档和README已把主路径改为直接在线更新：现有1.x安装
只点击自动医生这一行的“更新”，全新安装填写仓库地址并将分支或标签留空。无需加载
分支列表或导入ZIP。线上回滚不让客户端倒退Git历史，而是由维护者从更新前备份 tree
在当前 `main` 上发布向前回滚提交。该修订没有修改生产运行时，并由版本一致性测试
阻止说明回退为“只能本地ZIP安装”或重新要求候选分支切换。离线包仍作为维护者复核
产物保留，因说明文件进入包白名单而重建，SHA-256已同步更新。
...
这些项目不改变 `2.0.0-rc.1` 的完成门；正式 `2.0.0` 仍需后续维护者审阅决定。
