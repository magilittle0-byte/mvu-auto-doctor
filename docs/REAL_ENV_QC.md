...
2. Observe the variable record advance through `repairing` and `state-committing`.
   These phases gate only doctor-managed variable writes and explicit opt-in
   settled readers. The database, continuity engine, forum and actor shards may read
   the same accepted `<content>` snapshot independently; they must not wait on or
   mutate the variable task.
3. Require the state transaction write-ahead record to be durable before the
   ...
   `settled`.
4. Call compatibility API v4 `runAfterTargetSettled` as an explicit settled-only
   doctor-managed client. Its reader must receive the accepted narrative and
   fingerprint only after `settled`. Repeat with regenerate, new swipe, chat switch,
   failure, and cancellation; each old target must return
   `abandoned/stale/failed` and produce zero doctor-managed writes. Calling
   `waitForTargetSettled` without `waitForVariable: true` is an independent-reader
   path and must return promptly instead of creating a database barrier.
...
7. Exercise the internal V2 database-safety gate with 600 and 601 Unicode characters,
   parameterized and concatenated SQL, and matching/conflicting revisions. This
   synthetic gate is not TavernDB orchestration. The 601 + non-parameterized +
   revision-conflict case must report all three issues and perform zero commits.
